...
pack:
	$(HEREROCKS_ACTIVE) && eval $$(luarocks path) && \
		luarocks pack ulf.process-$(ROCKS_PACKAGE_VERSION)-$(ROCKS_PACKAGE_REVISION).rockspec "$(ROCKS_PACKAGE_VERSION)-$(ROCKS_PACKAGE_REVISION)" 

publish:
	$(HEREROCKS_ACTIVE) && eval $$(luarocks path) && \
		luarocks upload ulf.process-$(ROCKS_PACKAGE_VERSION)-$(ROCKS_PACKAGE_REVISION).rockspec ulf.process--$(ROCKS_PACKAGE_VERSION)-$(ROCKS_PACKAGE_REVISION).src.rock  --api-key=$(LUAROCKS_API_KEY)
...
$(LUAROCKS_DEPS): $(LUAROCKS) $(BUSTED_HTEST) $(NLUA)
	...
	@$(HEREROCKS_ACTIVE) && eval $$(luarocks path) && \
	luarocks make ulf.process-$(ROCKS_PACKAGE_VERSION)-$(ROCKS_PACKAGE_REVISION).rockspec && \
	luarocks test --prepare ulf.process-$(ROCKS_PACKAGE_VERSION)-$(ROCKS_PACKAGE_REVISION).rockspec && \
